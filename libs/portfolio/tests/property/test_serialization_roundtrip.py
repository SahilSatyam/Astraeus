...
survive a model_dump() → model_validate() round-trip without data loss or mutation.
"""
...
class TestTargetPortfolioSerialization:
    """Property 21: TargetPortfolio serialization round-trip.

    **Validates: Requirements 17.1, 17.6**

    For any valid TargetPortfolio instance, serializing via model_dump()
    and deserializing via model_validate() produces an identical object.
    """

    @given(portfolio=st_target_portfolio())
    @settings(max_examples=200, deadline=None)
    def test_roundtrip_preserves_equality(self, portfolio: TargetPortfolio) -> None:
...
class TestRiskReportSerialization:
    ...
    **Validates: Requirements 17.2, 17.6**
    ...
    For any valid RiskReport instance, serializing via model_dump()
    and deserializing via model_validate() produces an identical object.
    """

    @given(report=st_risk_report())
    @settings(max_examples=200, deadline=None)
    def test_roundtrip_preserves_equality(self, report: RiskReport) -> None:
    ...
    @given(report=st_risk_report())
    @settings(max_examples=100, deadline=None)
    def test_roundtrip_preserves_types(self, report: RiskReport) -> None:
